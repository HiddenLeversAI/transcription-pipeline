...
const initRetryColumns = async (db) => {
    try {
        await db.prepare(`
...
function estimateAudioDuration(fileSize, filename) {
...
app.post('/upload/signed-url', authenticate, async (c) => {
    ...
    `).first();
        const usedMinutes = currentMonthUsage?.used_minutes || 0;
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).bind(job.id, job.filename, job.fileSize, '', job.status, job.estimatedCost, job.createdAt).run();
        return c.json({
            uploadUrl,
            jobId,
            key,
...
app.post('/upload/*', async (c) => {
    ...
    `).bind(jobId).first();
        if (jobResult) {
            ...
            const estimatedDuration = estimateAudioDuration(body.byteLength, jobResult.filename);
            await sendJobCreatedToAirtable(c.env, jobId, {
                filename: jobResult.filename,
                fileSize: body.byteLength,
                fileType: contentType,
                estimatedCost: jobResult.estimated_cost || 0,
                estimatedDuration,
...
app.post('/upload/complete', authenticate, async (c) => {
    ...
    `).bind(jobId).first();
        if (jobResult) {
            // Send job creation notification to Airtable now that file is actually uploaded
            const estimatedDuration = estimateAudioDuration(fileSize, jobResult.filename);
            await sendJobCreatedToAirtable(c.env, jobId, {
                filename: jobResult.filename,
                fileSize: fileSize,
                fileType: fileType,
...
app.get('/job/:jobId/status', authenticate, async (c) => {
    ...
    `).bind(jobId).first();
        ...
        const job = {
            ...
            lastRetryAt: result.last_retry_at,
        ...
        `).bind(jobId).first();
                if (updatedResult) {
                    job.status = updatedResult.status;
                    job.transcriptionData = updatedResult.transcription_data ?
                        JSON.parse(updatedResult.transcription_data) : undefined;
                    job.completedAt = updatedResult.completed_at;
                }
            }
        }
        return c.json(job);
...
app.get('/usage/stats', authenticate, async (c) => {
    ...
    `).all();
        const stats = {
            currentMonth,
            monthlyStats: {
                totalJobs: monthlyStats?.total_jobs || 0,
                completedJobs: monthlyStats?.completed_jobs || 0,
                failedJobs: monthlyStats?.failed_jobs || 0,
                processingJobs: monthlyStats?.processing_jobs || 0,
                totalCost: Math.round((monthlyStats?.total_cost || 0) * 100) / 100,
                totalMinutes: Math.round((monthlyStats?.total_minutes || 0) * 10) / 10,
                avgProcessingTimeMs: Math.round(monthlyStats?.avg_processing_time_ms || 0),
                remainingMinutes: MONTHLY_USAGE_LIMIT_MINUTES - (monthlyStats?.total_minutes || 0),
                usagePercentage: Math.round(((monthlyStats?.total_minutes || 0) / MONTHLY_USAGE_LIMIT_MINUTES) * 100)
            },
            dailyBreakdown: dailyBreakdown.results.map((day) => ({
                date: day.date,
                jobs: day.jobs,
...
app.post('/job/:jobId/retry', authenticate, async (c) => {
    ...
    `).bind(jobId).run();
        ...
        const jobIdPart = result.id.replace('job-', '');
        const filename = result.filename.replace(/\s+/g, '_');
...
app.post('/debug/test-upload-complete', async (c) => {
    ...
    `).bind(testPayload.jobId).first();
        ...
        const estimatedDuration = estimateAudioDuration(testPayload.fileSize, jobResult.filename);
...
app.post('/webhook/salad-debug', async (c) => {
    ...
    `).bind(payload.job_id).first();
        ...
        await updateJobFromSalad(c.env, result.id, payload);
...
async function shouldRetryError(error) {
...
async function getRetryDelay(retryCount) {
    // Exponential backoff: base delay * 2^retryCount, capped at max
    const delay = Math.min(RETRY_DELAY_BASE_MS * Math.pow(2, retryCount), RETRY_DELAY_MAX_MS);
    // Add jitter to prevent thundering herd
    const jitter = Math.random() * 1000;
    return delay + jitter;
}
async function scheduleRetry(env, jobId, fileKey, retryCount) {
    ...
    WHERE id = ?
  `).bind(retryCount, new Date().toISOString(), `Retrying in ${Math.round(delay / 1000)} seconds (attempt ${retryCount + 1}/${MAX_RETRY_ATTEMPTS})`, jobId).run();
...
async function startTranscription(env, jobId, fileKey) {
    try {
    ...
    `).bind(jobId).first();
        const currentRetryCount = jobResult?.retry_count || 0;
        // Check if we should retry this error
        const shouldRetry = await shouldRetryError(error);
...
async function checkSaladStatus(env, saladJobId) {
    try {
...
async function updateJobFromSalad(env, jobId, saladData) {
    try {
        ...
        WHERE id = ?
      `).bind(JSON.stringify(transcriptionData), new Date().toISOString(), actualCost, processingTimeMs, jobId).run();
...
async function backupTranscriptToR2(env, jobId, transcriptionData, saladData) {
    try {
        const now = new Date();
        ...
        await env.R2_BUCKET.put(`${basePath}/transcript.json`, JSON.stringify({
            jobId,
            transcriptionData,
            saladResponse: {
        ...
        await env.R2_BUCKET.put(`${basePath}/transcript.txt`, transcriptionData.text, {
            httpMetadata: {
        ...
        if (saladData.captions || (transcriptionData.segments && transcriptionData.segments.length > 0)) {
            ...
            await env.R2_BUCKET.put(`${basePath}/transcript.srt`, srtContent, {
                httpMetadata: {
                    contentType: 'text/plain',
                },
                customMetadata: {
...
async function sendJobCreatedToAirtable(env, jobId, jobData) {
    try {
...
async function sendToAirtable(env, jobId, saladData) {
    try {
        // Get job details
        const jobResult = await env.DB.prepare(`
      SELECT * FROM transcription_jobs WHERE id = ?
